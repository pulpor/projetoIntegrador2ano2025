...
// cache de usuários pendentes (do backend) para uso em contadores/estado
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  (async () => {
    try {
      ...
      // fallback: manter turmas já existentes no localStorage
  ...
  // Carregar aprovados primeiro (para popular contadores) e depois pendentes
  try { await loadApprovedStudents(); } catch (e) { console.warn('Erro ao carregar aprovados:', e); }
  try { await loadPendingUsers(); } catch (e) { console.warn('Erro ao carregar pendentes:', e); }
...
function getTurmas() {
  // Prefer server-side turmas; fallback para localStorage
  const masterUsername = localStorage.getItem('username');
  if (!masterUsername) return [];
  try {
    // chamada síncrona não é possível aqui, então usamos valor em cache salvo no DOM via dataset
...
function addTurma(nome) {
  // Tenta salvar no backend; se falhar, salva no localStorage
  (async () => {
    try {
      const res = await apiRequest('/usuarios/turmas', {
        method: 'POST',
      ...
      // atualizar cache no DOM para getTurmas pegar
      document.body.dataset.turmasCache = JSON.stringify(res.turmas || []);
      localStorage.setItem('turmas_' + localStorage.getItem('username'), JSON.stringify(res.turmas || []));
    } catch (err) {
      const masterUsername = localStorage.getItem('username');
...
function renderTurmas() {
  ...
  if (!lista) return;
  // Calcular contagem por turma usando preferencialmente o campo 'class' (o JSON usa 'class')
  // Normalizamos nomes (trim + lowercase) para evitar problemas com espaços/case
  const alunosPorTurma = {};
  const usersByTurma = {};
  // Normaliza: trim, lowercase e remove acentuação para comparações mais robustas
  const normalize = (v) => {
    if (!v) return '';
    try {
      return v.toString().trim().toLowerCase().normalize('NFD').replace(/\p{Diacritic}/gu, '');
    } catch (e) {
      // fallback sem Unicode property
  ...
  (originalStudents || []).forEach(s => addUser(s));
  ...
  // Adicionar evento de clique no card para abrir modal com detalhes da turma
  lista.querySelectorAll('.student-card').forEach(card => {
    card.addEventListener('click', (e) => {
      // evitar o clique quando clica no botão de remover
...
// Retorna usuários aprovados + pendentes para uma turma (normalizando nomes)
function getUsersForTurma(turmaName) {
  const normalize = (v) => {
    if (!v) return '';
    try {
      return v.toString().trim().toLowerCase().normalize('NFD').replace(/\p{Diacritic}/gu, '');
  ...
  const all = [];
  (originalStudents || []).forEach(s => { if (normalize(s.class || s.turma) === key) all.push(s); });
  (pendingUsersCache || []).forEach(p => { if (normalize(p.class || p.turma) === key) all.push(p); });
  return all;
}

function openTurmaModal(turmaName) {
...
async function loadPendingUsers() {
  try {
    ...
    // Log básico para ajudar no diagnóstico
  ...
  // armazenar pendentes em cache para uso nos contadores de turmas
  pendingUsersCache = allUsers || [];
  console.log('[FRONTEND] Dados recebidos de pending-users:', allUsers, 'masterUsername:', masterUsername);
...
async function loadApprovedStudents() {
  ...
  // atualizar turmas pois contagem depende dos alunos aprovados
  renderTurmas();
...
function renderPendingUsersSpecifique(users) {
  ...
  container.innerHTML = '';
  const turmas = getTurmas();
  console.log('[FRONTEND] renderPendingUsersSpecifique - users:', users, 'turmas:', turmas);
...
async function approveUser(userId) {
  try {
    // turma obrigatória
    let turma = arguments.length > 1 ? arguments[1] : null;
